...
function* googleLogin() {
  const URL = `https://daodao-server.vercel.app/auth/google`;
  try {
...
function* userLogin() {
  const userURL = `https://daodao-server.vercel.app/user/current_user`;
  try {
...
function* userLogout() {
  const URL = `https://daodao-server.vercel.app/user/logout`;
  try {
    yield fetch(URL, {
      method: 'GET',
    }).then((res) => res.json());
...
function* userUpdate(action) {
  const URL = `https://daodao-server.vercel.app/user/update`;
